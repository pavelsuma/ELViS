...
from .matcher import init_weights, _BASE_URL, Matcher
from .transformer_dec import TransformerDecoder, TransformerDecoderLayer


@Matcher.register('ames')
class AMES(nn.Module):
    def __init__(self, desc_name, local_dim, model_dim=128, nhead=2,
        ...
        super(AMES, self).__init__(desc_name, local_dim, model_dim, binarized)
        ...
        if pretrained is not None:
            self.load_state_dict(torch.hub.load_state_dict_from_url(os.path.join(_BASE_URL, 'ames', 'networks', pretrained),
